...
class AlgorithmMonitor(AlgorithmSubscriber):
    ...
    def _create_layout(self, model: Problem) -> Layout:
        ...
        layout["stats"].split_column(
            self._create_stats_table(model), self._create_event_description()
        )
        return layout
    ...
    def _create_stats_table(self, model: Problem):
        ...
        rows[format_stat_name(TIME_UNTILL_OPTIMUM_FOUND)] = (
            f"{rows[format_stat_name(TIME_UNTILL_OPTIMUM_FOUND)]:>{pad_time}} s"
        )
        rows[format_stat_name(ACTIVE_TIME)] = (
            f"{rows[format_stat_name(ACTIVE_TIME)]:>{pad_time}} | {self._solver_config.time_limit} s"
        )
    ...
    def on_solution(self, model: Problem, solution: State):
        self._update_last_event(
            Event(
                name="Solution found", message=f"Found solution for problem: {solution}"
            )
        )
        self._update_live_display(self._create_layout(model))
        self._live.stop()
