...
CURR_NEIGHBOUR = "current_neighbour"
...
class VisualizationSubscriber(AlgorithmSubscriber):
    ...
    def __init_subclass__(cls):
        VisualizationSubscriber.visualizations[cls.get_corresponding_problem()] = cls
    ...
    def _init_pygame(self):
        ...
        self.state_screens = {
            ...
            CURR_NEIGHBOUR: self.canvas.subsurface(
                pygame.Rect(
                    *self.screen_coords[CURR_NEIGHBOUR],
                    self._SCREEN_SIZE[0] / 2,
                    self._SCREEN_SIZE[1] / 2,
                )
            ),
    ...
    def _init_state(self):
        ...
        self.states = {BEST_STATE: None, CURR_NEIGHBOUR: None, FROM_STATE: None}

    def on_next_neighbour(
        self, model: Problem, from_state: State, next_neighbour: State
    ):
    ...
    def _draw_border(self):
        rect = pygame.Rect(
            0, 0, self.main_screen.get_width() - 1, self.main_screen.get_height() - 1
        )
        pygame.draw.rect(self.main_screen, BLACK, rect, 4)
        pygame.draw.line(
            self.main_screen,
            BLACK,
            (self.main_screen.get_width() / 2, 0),
            (self.main_screen.get_width() / 2, self.main_screen.get_height()),
            3,
        )
        pygame.draw.line(
            self.main_screen,
            BLACK,
            (0, self.main_screen.get_height() / 2),
            (self.main_screen.get_width(), self.main_screen.get_height() / 2),
            3,
        )
    ...
    def _draw_states(self, model: Problem):
        ...
        for state_name in self.states:
            state, screen, screen_coords = (
                self.states[state_name],
                self.state_screens[state_name],
                self.screen_coords[state_name],
            )
            if not state:
                continue
    ...
    def _draw_text_with_background(
        self, screen, text, position, font_size, bold, state_name
    ):
        length = len(state_name)
        rect = pygame.Rect(
            font_size - 5, font_size + 3, font_size * length / 2, font_size
        )
    ...
    def _draw_information(self, model: Problem):
        ...
        for idx, item in enumerate(stats.items()):
            ...
            self._draw_text(
                screen,
                f"{stat_name.replace('_', ' ')}: {stat_value}".capitalize(),
                (
        ...
        rect = pygame.Rect(
            screen.get_width() / 2 - 4.5 * self._FONT_SIZE,
            self._FONT_SIZE / 2,
    ...
    def _draw_button(self):
        screen = self.state_screens[STATS]
        rect = pygame.Rect(
            screen.get_width() / 3, screen.get_height() / 2, *self._BUTTON_SIZE
        )
        if self._is_freezed:
    ...
    def _is_button_clicked(self, position):
        ...
        return (
            screen.get_width() / 3
            <= position[0] - self._SCREEN_SIZE[0] / 2
            <= screen.get_width() / 3 + self._BUTTON_SIZE[0]
            and screen.get_height() / 2
            <= position[1] - self._SCREEN_SIZE[1] / 2
            <= screen.get_height() / 2 + self._BUTTON_SIZE[1]
        )
