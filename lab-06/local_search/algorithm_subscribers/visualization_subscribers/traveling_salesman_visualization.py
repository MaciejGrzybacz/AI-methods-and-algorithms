...
class TravelingSalesmanStateDrawer(StateDrawer):
    def draw_state(
        self, screen, model: TravelingSalesmanProblem, state: TravelingSalesmanState
    ):
        screen.fill(WHITE)
        self._draw_buildings(model, state, screen)
        route = [(self._scale(model.points[idx], model, screen)) for idx in state.route]
        pygame.draw.lines(screen, ROAD_COLOR, True, route, 3)
    ...
    def _draw_buildings(
        self, model: TravelingSalesmanProblem, state: TravelingSalesmanState, screen
    ):
        if not state:
            return
        for idx in state.route:
            ...
            screen.blit(building, (x - pic_size / 2, y - pic_size / 2))
