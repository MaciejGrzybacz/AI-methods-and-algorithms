...
class GraphColoringStateDrawer(StateDrawer):
    ...
    def _generate_coords(model: GraphColoringProblem):
        ...
        for _ in range(model.n_vertices):
            while (x, y) in coord:
                x, y = randint(0, model.n_vertices), randint(0, model.n_vertices)
            coord.append((x, y))

        return coord

    def draw_state(
        self, screen, model: GraphColoringProblem, state: GraphColoringState
    ):
        """
        Draws state
        """
        ...
        for i in range(len(graph)):
            for j in range(len(graph[i])):
                pygame.draw.line(
                    ...
                    self._scale(self.coords[graph[i][j]], screen, extremes),
                    3,
                )
    ...
    def _draw_vertices(
        ...
        for i in range(len(self.coords)):
            ...
            pygame.draw.circle(screen, colors[state.coloring[i].color], (x, y), 10)
            pygame.draw.circle(screen, (0, 0, 0), (x, y), 10, 2)

    def _get_colors(self, model: GraphColoringProblem):
        if not self._available_colors:
            self._available_colors = self._generate_random_colors(model.n_vertices)
    ...
    def _scale(
        self, point: Tuple[int, int], screen, extremes: Tuple[int, int, int, int]
    ):
