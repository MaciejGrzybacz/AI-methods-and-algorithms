from local_search.problems.limited_avatar_problem.moves.move_generator import (
    LimitedAvatarMoveGenerator,
)
...
class MutatePolygon(LimitedAvatarMoveGenerator):
    def _generate_coords_diff(self) -> Tuple[int, int]:
        return (
            random.randint(-self.im_size[0] // 2, self.im_size[0] // 2),
        ...
        while True:
            ...
            yield random.choice(
                [
                    ...
                    ChangeCoordinatesMove(
                        state,
                        polygon_idx,
                        self._generate_vertex_idx(state),
                        self._generate_coords_diff(),
                    ),
                ]
            )

    def random_moves(
        self, state: LimitedAvatarState
    ) -> Generator[Union["ChangeColorMove", "ChangeCoordinatesMove"], None, None]:
...
class ChangeCoordinatesMove(Move[LimitedAvatarState]):
    def __init__(
        self,
        from_state: LimitedAvatarState,
        polygon_idx: int,
        vertex_idx: int,
        coords_diff: Tuple[int, int],
    ):
        super().__init__(from_state)
        (self.polygon_idx, self.vertex_idx, self.coords_diff) = (
            polygon_idx,
            vertex_idx,
            coords_diff,
        )
    ...
    def make(self) -> LimitedAvatarState:
        ...
        changed_polygons[self.polygon_idx].change_coords(
            self.vertex_idx, self.coords_diff
        )
        return LimitedAvatarState(
            polygons=changed_polygons, image_size=self.state.image_size
        )


class ChangeColorMove(Move[LimitedAvatarState]):
    def __init__(
        self, from_state: LimitedAvatarState, polygon_idx: int, color_diff: Color
    ):
        super().__init__(from_state)
