...
class LimitedAvatarProblem(Problem):
    def __init__(
        ...
        move_generator_name = (
            move_generator_name
            or list(self.get_available_move_generation_strategies())[0]
        )
    ...
    def random_state(self) -> LimitedAvatarState:
        ...
        def _generate_vertex() -> Vertex:
            ...
            return Vertex(
                x=random.randint(0 - offset, self._image_size[0] + offset),
        ...
        polygons = [
            Polygon(
                vertices=[
                    _generate_vertex() for _ in range(self.config.n_polygon_vertices)
                ],
                color=_generate_color(),
            )
            for _ in range(self.config.n_polygons)
        ]
    ...
    def from_benchmark(
        ...
        img = Image.open(
            Path(local_search.__file__).parent
            / "problems"
            / "limited_avatar_problem"
            / "benchmarks"
            / benchmark_name
        )
        return LimitedAvatarProblem(img, move_generator_name, config)
