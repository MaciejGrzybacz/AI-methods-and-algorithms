...
class GraphColoringProblem(Problem):
    def __init__(
        ...
        move_generator_name = (
            move_generator_name
            or list(GraphColoringMoveGenerator.move_generators.keys())[0]
        )
        move_generator = GraphColoringMoveGenerator.move_generators[
            move_generator_name
    ...
    @classmethod
    def from_benchmark(
        cls, benchmark_name: str, move_generator_name: str = None, goal_name: str = None
    ):
    ...
    def asdict(self):
        base = super().asdict()
        return {"edges": [(edge.start, edge.end) for edge in self.edges], **base}

    @classmethod
    def from_dict(cls, data):
        data["edges"] = [
            Edge(start=edge_tuple[0], end=edge_tuple[1]) for edge_tuple in data["edges"]
        ]
        return cls(**data)
