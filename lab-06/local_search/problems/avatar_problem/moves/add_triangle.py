...
from local_search.problems.avatar_problem.moves.move_generator import (
    AvatarMoveGenerator,
)
...
class AddTriangle(AvatarMoveGenerator):
    def _generate_vertex(self) -> Vertex:
        return Vertex(
            x=randint(0 - self.offset, self.im_size[0] + self.offset),
    ...
    def _generate_color(self) -> Color:
        return Color(
            R=randint(0, 255), G=randint(0, 255), B=randint(0, 255), A=randint(0, 255)
        )

    def random_moves(
        self, state: AvatarState
    ) -> Generator["AddTriangleMove", None, None]:
...
class AddTriangleMove(Move[AvatarState]):
    def __init__(self, from_state: AvatarState, vertices: List[Vertex], color: Color):
        ...
        (self.coordinates, self.color) = (
            self._to_coordinates(vertices),
            self._to_color_coordinates(color),
        )
