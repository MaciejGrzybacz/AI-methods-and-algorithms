...
from local_search.problems.traveling_salesman_problem.models.edge import Edge
...
class TravelingSalesmanState(State):
    ...
    def __str__(self):
        return str.join(
            " -> ",
            map(
                lambda idx: f"({self.points[idx].x}, {self.points[idx].y})", self.route
            ),
        )

    @property
    def edges(self) -> Iterable[Edge]:
        not_connected_edges = zip(self.route, self.route[1:] + [self.route[0]])
