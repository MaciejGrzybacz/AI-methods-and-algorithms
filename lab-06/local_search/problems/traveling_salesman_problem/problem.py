...
from local_search.problems.traveling_salesman_problem.goal import TravelingSalesmanGoal
...
class TravelingSalesmanProblem(Problem):
    def __init__(
        ...
        move_generator = TravelingSalesmanMoveGenerator.move_generators[
            move_generator_name
        ]()
    ...
    def from_benchmark(
        cls,
        benchmark_name: str,
        move_generator_name: str = None,
        goal_name: str = "distance",
    ):
    ...
    def parse_model(cls, file_buffer: TextIOWrapper):
        ...
        points = list(map(line_to_point, [line for line in file_buffer]))
    ...
    def from_dict(cls, data):
        data["points"] = [
            Point(x=point_tuple[0], y=point_tuple[1]) for point_tuple in data["points"]
        ]
        return cls(**data)
