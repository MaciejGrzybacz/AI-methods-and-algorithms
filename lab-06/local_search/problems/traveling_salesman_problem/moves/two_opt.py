...
class TwoOptMove(Move[TravelingSalesmanState]):
    ...
    def make(self) -> TravelingSalesmanState:
        new_route = (
            self.state.route[0 : self.i1]
            + list(reversed(self.state.route[self.i1 : self.i2]))
            + self.state.route[self.i2 :]
        )
...
class TwoOpt(TravelingSalesmanMoveGenerator):
    def available_moves(
        self, state: TravelingSalesmanState
    ) -> Generator[Move[TravelingSalesmanState], None, None]:
