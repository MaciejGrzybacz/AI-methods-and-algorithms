...
from local_search.problems.traveling_salesman_problem.models.salesman import Salesman
...
class Distance(TravelingSalesmanGoal):
    def objective_for(self, state: TravelingSalesmanState) -> int:
        route = [self._points[i] for i in state.route]
        return int(Salesman.from_point(route[0]).walk_route(route).travelled_distance)
