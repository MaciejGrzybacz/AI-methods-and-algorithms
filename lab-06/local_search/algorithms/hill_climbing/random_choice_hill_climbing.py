...
class RandomChoiceHillClimbing(HillClimbing):
    ...
    def _climb_the_hill(self, model: Problem, state: State) -> Union[State, None]:
        ...
        random_neighbors_iterator = self._get_random_neighbours(model, state)

        random_neighbor = next(random_neighbors_iterator)
            
        if model.improvement(state, random_neighbor) > 0:
