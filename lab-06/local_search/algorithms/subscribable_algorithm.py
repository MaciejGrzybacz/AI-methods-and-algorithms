...
class SubscribableAlgorithm(Algorithm):
    ...
    def __init__(self, config: AlgorithmConfig = None):
        super().__init__(
            best_obj=None, best_state=None, config=config or DEFAULT_CONFIG
        )
        ...
        for _ in range(how_much):
            try:
                perturbed_state = next(
                    model.move_generator.random_moves(perturbed_state)
                ).make()
    ...
    def _get_neighbours(
        self, model: Problem, state: State
    ...
    def next_state(self, model: Problem, state: State) -> Union[State, None]:
        ...
        if self._is_stuck_in_local_optimum():
            ...
            self._on_local_optimum_escape(model, from_state=state, to_state=next_state)
    ...
    def subscribe(
        self, subscriber: TSubscriber, niceness=0
    ) -> Subscription[TSubscriber]:
        subscription = Subscription(
            niceness=niceness, subscriber=subscriber, subscribable=self
        )
