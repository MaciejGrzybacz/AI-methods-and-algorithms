...
def get_or_prompt_if_not_exists_or_invalid(
    options, option_key: str, option_config=None
) -> str:
    """
    ...
    if option_config.setdefault("type", False):
        if (
            isinstance(option_config["type"], click.Choice)
            and options[option_key] not in option_config["type"].choices
        ):
            console.print(
...
def prompt(prompt, config):
    if config.setdefault("type", False) and config["type"] is bool:
        return click.confirm(prompt, default=config.setdefault("default", False))
    else:
        return click.prompt(
            prompt,
            type=config.setdefault("type", None),
            default=config.setdefault("default", None),
        )
