...
def create_dataclass(options, dataclass: Type):
    ...
    for field in fields(dataclass):
        if issubclass(field.type, Enum):
            value = get_or_prompt_if_not_exists_or_invalid(
                options, field.name, {"default": field.default.value}
            )
            field_value = field.type(value)
        else:
            field_value = get_or_prompt_if_not_exists_or_invalid(
                options, field.name, {"default": field.default, "type": field.type}
            )
