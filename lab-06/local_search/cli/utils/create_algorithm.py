...
def create_algorithm(problem_model: Problem, options) -> SubscribableAlgorithm:
    ...
    algo_name = assure_problem_is_solvable_by_algo(config, "name", problem_model)
    ...
    get_or_prompt_if_not_exists_or_invalid(
        ...
        {
            "type": click.Choice(
                list(SubscribableAlgorithm.algorithms.keys()), case_sensitive=True
            )
        },
    )
    ...
    add_algorithm_subscribers(options, problem_model, algorithm)
    return algorithm
...
def assure_problem_is_solvable_by_algo(config, key: str, problem_model: Problem):
    ...
    algo_name = get_or_prompt_if_not_exists_or_invalid(
        config,
        key,
...
def add_algorithm_subscribers(
    options, problem_model: Problem, algorithm: SubscribableAlgorithm
):
    if options.setdefault("visualization", {}).setdefault("enabled", False):
        add_visualization_subscriber(options["visualization"], problem_model, algorithm)


def add_visualization_subscriber(
    options, problem_model: Problem, algorithm: SubscribableAlgorithm
) -> None:
    print_section_name("Configuring visulization")
    visualization = VisualizationSubscriber.visualizations.setdefault(
        type(problem_model), None
    )
    if visualization:
        visualization_params = {"model": problem_model}
        config_type = get_type_for_param(visualization, "config")
        if config_type:
            visualization_params["config"] = create_dataclass(options, config_type)
