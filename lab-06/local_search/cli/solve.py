...
def solve(config_file, **cli_options):
    ...
    **Each** configuration file should have a following sections (headers have the same name as key in the configuration file).
    ...
    if get_or_prompt_if_not_exists_or_invalid(
        options, "save_solution", {"type": bool, "default": True}
    ):
        path = create_path_to_save_solution(options)
        while path.exists():
            should_save = click.confirm(
                "There is already a solution with to problem with this name, benchmark, move generator and goal. Do you want to save this solution?",
                default=True,
            )
            if not should_save:
                return
            ...
            if not should_overwrite:
                new_name = click.prompt(
                    f"New name to save in folder {path.parent} (without extension)",
                    type=str,
                )
