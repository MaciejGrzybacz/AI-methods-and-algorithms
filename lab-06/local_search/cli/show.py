...
@click.argument(
    "path_to_solution", type=click.Path(exists=True, readable=True), required=True
)
def show(path_to_solution: str):
    solution = Solution.from_json(Path(path_to_solution))
    state_drawer = VisualizationSubscriber.visualizations[
        type(solution.problem)
    ].create_state_drawer(solution.problem)
