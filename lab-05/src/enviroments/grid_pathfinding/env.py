...
from src.enviroments.grid_pathfinding.problem.grid import GridCell, GridCoord
from src.enviroments.grid_pathfinding.problem.grid_pathfinding import GridPathfinding, Grid, GridCoord, GridMove
...
class GridPathfindingEnv(gym.Env):
    ...
    def step(self, action: int) -> tuple[dict[str, GridCoord], float, bool, dict[str, float]]:
        ...
        bait = self.problem.is_bait(self._agent_location)
        done = goal or mine or bait
        reward = self._get_reward(old_location, self._agent_location, not valid_move, goal, mine, bait)
        info = self._get_info()
    ...
    def _get_reward(self, old_location: GridCoord, new_location: GridCoord, invalid_move: bool, goal: bool, mine: bool, bait: bool):    
        if mine:
            return -100
        if goal:
            return 100
    ...
    def reset(self, seed=None, return_info=True, options=None) -> tuple[dict[str, GridCoord], dict[str, any]]:
        ...
        return (observation, info) if return_info else (observation, info)
