...
class Match(Generic[TPlayer, TOpponent]):
    ...
    def play(self) -> TPlayer | TOpponent | None:
        while not self._is_match_end():
            ...
            self.print_progress(
                f"{self._players_names[self.current_player]}'s turn. ({self._players[self.current_player].name()})")
            for player, bot in self._players.items():
                if isinstance(bot, HeuristicBot):
                    self.print_progress(
                        f"- heuristic for '{self._players_names[player]}': {bot.heuristic(self.current_state, player)}")
    ...
    def _next_move(self, player: TPlayer | TOpponent) -> TState:
        ...
        self.print_progress(
            f"Move took {(datetime.now() - start).total_seconds()}s. Time out: {to_ctx_mgr.state == to_ctx_mgr.TIMED_OUT}")
        ...
        if action == None:
            ...
            self.print_progress(
                f"Bot {player.name()} chose an illegal action {action}, will be overriden with a random one")
            action = random.choice(available_actions)
