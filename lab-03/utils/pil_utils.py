...
class GridDrawer:
    def __init__(self, image: Image.Image, grid, header_height: int | None = None):
        self.image = image
    ...
    def get_cell_coords(self, padding: int, x: int, y: int, grid_x_end: int | None = None, grid_y_end: int | None = None, have_border: bool = True):
        if have_border:
            border = self.border
        else:
            border = 0
    ...
    def draw_rectangle(self, grid_coords: tuple[int, ...], fill: tuple[int, int, int] | None = None, padding=0):
    ...
    def draw_circle(self, grid_x: int, grid_y: int, fill: tuple[int, int, int] | None = None):
        coords = self.get_cell_coords(0, grid_x, grid_y)
        self.draw.ellipse(coords, fill)

    def draw_dot(self, grid_x: int, grid_y: int, fill: tuple[int, int, int] | None = None):
