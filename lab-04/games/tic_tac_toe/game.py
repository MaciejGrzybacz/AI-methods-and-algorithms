...
class TicTacToeGame(Game[TicTacToeState, TicTacToeAction], ABC):
    ...
    def win_length(self) -> int:
        '''How long has be the line to win'''

    @property
    @abstractmethod
    def loose_length(self) -> int:
        '''How long has be the line to loose'''
    ...
    def current_score(self, state: TicTacToeState) -> Tuple[Score, Optional[TicTacToeLine]]:
    ...
    def to_ascii(self, state: TicTacToeState) -> str:
        ...
        actions = set() if is_terminal else set((a.x, a.y) for a in self.actions_for(state, state.current_sign.to_player()))
    ...
    def to_image(self, state: TicTacToeState, size: Tuple[int, int] = (800, 900)) -> Image.Image:
        ...
        line_colors = { TicTacToeSign.X: (242, 151, 159), TicTacToeSign.O: (151, 184, 190), TicTacToeSign.BLANK: (146, 98, 137) }
    ...
    def _get_winner(self, score: Score) -> Optional[Player]:
        if score == Score.WON:
            return Player.MAX
...
class TicTacToeGameLogic(TicTacToeGame):
    ...
    def __init__(self, board_size: int, win_length: int, loose_length: Optional[int] = None):
